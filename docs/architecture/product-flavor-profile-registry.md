...
- Покрыто 75 из 86-87 `RESOLVED` продуктов (батчи 1–5 по 15); остальные — предмет следующего, последнего батча.
- Целевая модель — 7 из 18 будущих Prisma-полей `Flavor` (ADR-014); остальные 11 явно исключены из registry, а не отложены. Все 75 уже покрытых продуктов доведены до полной 7-полевой модели (243 из 525 возможных значений заполнено); для Hook и Husky `heatResistance` по-прежнему не найден ни для одного продукта во всех батчах, `juiciness` заполнен только там, где источник явно использует слово «сочный».
