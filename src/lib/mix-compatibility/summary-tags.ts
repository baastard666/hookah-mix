...
export function calculateSummaryTags(result:MixProfileResult,profileBalance:AnalysisBlock,intensityBalance:AnalysisBlock):SummaryTag[]{
  ...
  // ADR-015/ADR-017: any of the 18 sensory fields may be null ("not measured") - the tag is simply not
  // added, it is never treated as "measured and low".
  if(p.sweetness!==null&&p.sweetness>=6.5)tags.add("SWEET");if(p.acidity!==null&&p.acidity>=6.5)tags.add("SOUR");if(p.creaminess!==null&&p.creaminess>=6)tags.add("CREAMY");if(p.cooling!==null&&p.cooling>=6)tags.add("COOLING");if(p.freshness!==null&&p.freshness>=6)tags.add("FRESH");if(p.dessertLevel!==null&&p.dessertLevel>=6)tags.add("DESSERT");if(p.spiceLevel!==null&&p.spiceLevel>=6)tags.add("SPICY");if(p.herbalLevel!==null&&p.herbalLevel>=6)tags.add("HERBAL");if(p.floralLevel!==null&&p.floralLevel>=6)tags.add("FLORAL");if(p.smokyLevel!==null&&p.smokyLevel>=6)tags.add("SMOKY");if(p.intensity!==null&&p.intensity>=7.5)tags.add("INTENSE");if(p.intensity!==null&&p.intensity<=4)tags.add("LIGHT");if(profileBalance.score>=8&&intensityBalance.score>=8)tags.add("BALANCED");
