...
describe("вклад и классификация нот",()=>{
 ...
 it("29. ограничивает dominantNotes тремя",()=>{const fake=Array.from({length:6},(_,i)=>({noteIds:[i],noteName:`n${i}`,noteSlug:`n${i}`,category:"OTHER" as const,contributionScore:10-i,sharePercent:25,sources:[],componentFlavorIds:[i]}));expect(classifyNotes(fake).dominantNotes).toHaveLength(3)});
 it("30. ограничивает secondaryNotes пятью",()=>{const fake=Array.from({length:9},(_,i)=>({noteIds:[i],noteName:`n${i}`,noteSlug:`n${i}`,category:"OTHER" as const,contributionScore:20-i,sharePercent:i===0?30:10,sources:[],componentFlavorIds:[i]}));expect(classifyNotes(fake).secondaryNotes).toHaveLength(5)});
 it("31. ограничивает backgroundNotes пятью",()=>{const fake=Array.from({length:12},(_,i)=>({noteIds:[i],noteName:`n${i}`,noteSlug:`n${i}`,category:"OTHER" as const,contributionScore:20-i,sharePercent:i===0?30:2,sources:[],componentFlavorIds:[i]}));expect(classifyNotes(fake).backgroundNotes).toHaveLength(5)});
