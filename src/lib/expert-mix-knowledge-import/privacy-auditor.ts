...
export const auditImportedKnowledgePrivacy = (records: readonly PublicExpertMixKnowledgeRecord[], privateTokens: readonly string[] = []): readonly ExpertMixImportIssue[] => {
  ...
  for (const forbidden of ["internalLabel", "sourceId", "rowNumber", "Imported from", ...privateTokens].filter(Boolean)) if (json.toLocaleLowerCase("ru-RU").includes(forbidden.toLocaleLowerCase("ru-RU"))) issues.push({ code: "PUBLIC_PRIVACY_LEAK", severity: "ERROR", field: forbidden, message: `Public output содержит приватный token: ${forbidden}.` });
