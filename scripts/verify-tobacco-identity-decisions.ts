...
import {
  ...
  createTobaccoIdentityReviewPlan, mapTobaccoIdentityDecisionToPublic, P0_IDENTITY_DECISIONS_BATCH_1, P0_IDENTITY_DECISIONS_BATCH_2, P0_IDENTITY_DECISIONS_BATCH_3, P0_IDENTITY_DECISIONS_BATCH_4, P0_IDENTITY_DECISIONS_BATCH_5,
...
const main = async (): Promise<void> => {
  ...
  assert.equal(P0_IDENTITY_DECISIONS_BATCH_1.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_2.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_3.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_4.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_5.length, 15); assert.equal(TOBACCO_IDENTITY_DECISIONS.length, 95); assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getByStatus("RESOLVED").length, 86); assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getByStatus("AMBIGUOUS").length, 6); assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getByStatus("UNRESOLVED").length, 2);
  assert.equal(p0Records.length, 140); assert.equal(p0Coverage.appliedDecisionCount, 126); assert.equal(p0Coverage.skippedDecisionCount, 14); assert.equal(p0Coverage.resolvedDelta, 126); assert.equal(p0Coverage.unresolvedDelta, -126); assert.equal(p0Coverage.manufacturerOnlyDelta, 0);
  ...
  assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getBySourceGroupId("identity-group-0023").status, "NOT_FOUND");
  ...
  console.log(JSON.stringify({ p0Batch5: { decisions: P0_IDENTITY_DECISIONS_BATCH_5.length, resolvedDecisions: P0_IDENTITY_DECISIONS_BATCH_5.filter(item => item.decision.status === "RESOLVED").length, ambiguousDecisions: P0_IDENTITY_DECISIONS_BATCH_5.filter(item => item.decision.status === "AMBIGUOUS").map(item => item.sourceIdentity.sourceGroupId), unresolvedDecisions: P0_IDENTITY_DECISIONS_BATCH_5.filter(item => item.decision.status === "UNRESOLVED").map(item => item.sourceIdentity.sourceGroupId), improvedVerifiedMixCount: p0Batch5ImprovedMixIds.length, improvedVerifiedMixIds: p0Batch5ImprovedMixIds, coverage: p0Batch5Coverage } }, null, 2));
