...
export function calculateSummaryTags(result:MixProfileResult,profileBalance:AnalysisBlock,intensityBalance:AnalysisBlock):SummaryTag[]{
  ...
  if(p.sweetness>=6.5)tags.add("SWEET");if(p.acidity>=6.5)tags.add("SOUR");if(p.creaminess>=6)tags.add("CREAMY");if(p.cooling>=6)tags.add("COOLING");if(p.freshness>=6)tags.add("FRESH");if(p.dessertLevel>=6)tags.add("DESSERT");if(p.spiceLevel>=6)tags.add("SPICY");if(p.herbalLevel>=6)tags.add("HERBAL");if(p.floralLevel>=6)tags.add("FLORAL");if(p.smokyLevel>=6)tags.add("SMOKY");if(p.intensity>=7.5)tags.add("INTENSE");if(p.intensity<=4)tags.add("LIGHT");if(profileBalance.score>=8&&intensityBalance.score>=8)tags.add("BALANCED");
