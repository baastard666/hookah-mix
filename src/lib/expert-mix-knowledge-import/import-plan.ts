...
import { resolveHeaderIndexes, rowsToRawEntities } from "./header-resolver";
...
export const createExpertMixImportPlan = (workbook: ExpertMixWorkbookData, inspection: ExpertMixWorkbookInspection): ExpertMixImportPlan => {
  ...
  for (const sheet of inspection.sheets) {
    ...
    for (const header of sheet.duplicateHeaders) issues.push({ code: "HEADER_DUPLICATED", severity: "ERROR", sheet: sheet.name, field: header, message: `Заголовок ${header} повторяется.` });
    const source = workbook.sheets.find(item => item.name === sheet.name); if (!source) continue;
    const canonical = resolveHeaderIndexes(source.rows[0]?.values ?? []);
