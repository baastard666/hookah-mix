...
import {
  ...
  createTobaccoIdentityReviewPlan, mapTobaccoIdentityDecisionToPublic, P0_IDENTITY_DECISIONS_BATCH_1, P0_IDENTITY_DECISIONS_BATCH_2,
...
const main = async (): Promise<void> => {
  ...
  assert.equal(P0_IDENTITY_DECISIONS_BATCH_1.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_2.length, 15); assert.equal(TOBACCO_IDENTITY_DECISIONS.length, 50); assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getByStatus("RESOLVED").length, 46); assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getByStatus("AMBIGUOUS").length, 3);
  assert.equal(p0Records.length, 61); assert.equal(p0Coverage.appliedDecisionCount, 55); assert.equal(p0Coverage.skippedDecisionCount, 6); assert.equal(p0Coverage.resolvedDelta, 55); assert.equal(p0Coverage.unresolvedDelta, -55); assert.equal(p0Coverage.manufacturerOnlyDelta, 0);
  assert.equal(p0Coverage.componentsAfter.resolved - p0Coverage.componentsBefore.resolved, 29); assert.equal(p0Coverage.catalogAfter.resolved - p0Coverage.catalogBefore.resolved, 26); assert.equal(p0Coverage.verifiedComponents.after.resolved - p0Coverage.verifiedComponents.before.resolved, 28);
  assert.equal(combinedCoverage.appliedDecisionCount, 92); assert.equal(combinedCoverage.skippedDecisionCount, 8); assert.equal(combinedCoverage.resolvedDelta, 92); assert.equal(combinedCoverage.manufacturerOnlyDelta, -37); assert.equal(combinedCoverage.unresolvedDelta, -55); assert.equal(combinedCoverage.invalidDecisionCount, 0); assert.equal(combinedCoverage.conflictCount, 0);
  ...
  assert.equal(canonicalProductIds.length, 46); assert.ok(canonicalProductIds.every(id => ASCII_CANONICAL_PRODUCT_ID_PATTERN.test(id))); assert.ok(canonicalProductIds.every(id => !/[^\x00-\x7F]/.test(id)));
