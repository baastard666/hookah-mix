...
const main = async (): Promise<void> => {
  ...
  assert.equal(canonicalProductIds.length, 34); assert.ok(canonicalProductIds.every(id => ASCII_CANONICAL_PRODUCT_ID_PATTERN.test(id))); assert.ok(canonicalProductIds.every(id => !/[^\x00-\x7F]/.test(id)));
