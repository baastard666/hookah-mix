...
- Покрыты 30 из 86-87 `RESOLVED` продуктов (батчи 1–2 по 15); остальные — предмет будущих итераций (batch 3+).
- Целевая модель — 7 из 18 будущих Prisma-полей `Flavor` (ADR-014); остальные 11 явно исключены из registry, а не отложены. Все 30 уже покрытых продуктов доведены до полной 7-полевой модели (113 из 210 возможных значений заполнено; для двух производителей — Hook, Husky — `heatResistance` не найден ни для одного продукта, `juiciness` заполнен только там, где источник явно использует слово «сочный»).
