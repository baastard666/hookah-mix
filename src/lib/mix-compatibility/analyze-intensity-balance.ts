...
export function analyzeIntensityBalance(input:MixCompatibilityInput):AnalysisBlock{
  ...
  if(dominant.influenceShare>=70)add("intensity.overdominant","caution",dominant.influenceShare>=80?-.8:-.5,dominant.influenceShare>=80?"Сильное доминирование":"Заметное доминирование",dominant.influenceShare>=80?"Один компонент может почти полностью перекрыть остальные":"Один компонент заметно сильнее остальных, но дополнительные ноты должны оставаться различимыми",[dominant.flavorId]);
