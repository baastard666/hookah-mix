...
const compare=(a:RawInfluence,b:RawInfluence):number=>b.score-a.score||b.component.percentage-a.component.percentage||b.component.profile.intensity-a.component.profile.intensity||a.component.flavorSlug.localeCompare(b.component.flavorSlug,"en")||String(a.component.flavorId).localeCompare(String(b.component.flavorId),"en");

export function calculateComponentInfluence(components:readonly MixProfileComponentInput[]):ComponentInfluence[]{
  const raw=components.map(component=>{const strongest=Math.max(...component.notes.filter(note=>note.noteType==="DOMINANT").map(note=>note.intensity));return{component,score:component.percentage*(0.6+component.profile.intensity/20)*(0.7+strongest/30)}}).sort(compare);
