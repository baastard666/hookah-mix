...
import {
  ...
  createTobaccoIdentityReviewPlan, mapTobaccoIdentityDecisionToPublic, P0_IDENTITY_DECISIONS_BATCH_1, P0_IDENTITY_DECISIONS_BATCH_2, P0_IDENTITY_DECISIONS_BATCH_3, P0_IDENTITY_DECISIONS_BATCH_4,
...
const main = async (): Promise<void> => {
  ...
  assert.equal(P0_IDENTITY_DECISIONS_BATCH_1.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_2.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_3.length, 15); assert.equal(P0_IDENTITY_DECISIONS_BATCH_4.length, 15); assert.equal(TOBACCO_IDENTITY_DECISIONS.length, 80); assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getByStatus("RESOLVED").length, 74); assert.equal(TOBACCO_IDENTITY_DECISION_REGISTRY.getByStatus("AMBIGUOUS").length, 5);
  assert.equal(p0Records.length, 121); assert.equal(p0Coverage.appliedDecisionCount, 111); assert.equal(p0Coverage.skippedDecisionCount, 10); assert.equal(p0Coverage.resolvedDelta, 111); assert.equal(p0Coverage.unresolvedDelta, -111); assert.equal(p0Coverage.manufacturerOnlyDelta, 0);
  assert.equal(p0Coverage.componentsAfter.resolved - p0Coverage.componentsBefore.resolved, 57); assert.equal(p0Coverage.catalogAfter.resolved - p0Coverage.catalogBefore.resolved, 54); assert.equal(p0Coverage.verifiedComponents.after.resolved - p0Coverage.verifiedComponents.before.resolved, 56);
  assert.equal(combinedCoverage.appliedDecisionCount, 148); assert.equal(combinedCoverage.skippedDecisionCount, 12); assert.equal(combinedCoverage.resolvedDelta, 148); assert.equal(combinedCoverage.manufacturerOnlyDelta, -37); assert.equal(combinedCoverage.unresolvedDelta, -111); assert.equal(combinedCoverage.invalidDecisionCount, 0); assert.equal(combinedCoverage.conflictCount, 0);
  ...
  assert.equal(canonicalProductIds.length, 74); assert.ok(canonicalProductIds.every(id => ASCII_CANONICAL_PRODUCT_ID_PATTERN.test(id))); assert.ok(canonicalProductIds.every(id => !/[^\x00-\x7F]/.test(id)));
  ...
  console.log(JSON.stringify({ p0Batch4: { decisions: P0_IDENTITY_DECISIONS_BATCH_4.length, resolvedDecisions: P0_IDENTITY_DECISIONS_BATCH_4.filter(item => item.decision.status === "RESOLVED").length, ambiguousDecisions: P0_IDENTITY_DECISIONS_BATCH_4.filter(item => item.decision.status === "AMBIGUOUS").map(item => item.sourceIdentity.sourceGroupId), improvedVerifiedMixCount: p0Batch4ImprovedMixIds.length, improvedVerifiedMixIds: p0Batch4ImprovedMixIds, coverage: p0Batch4Coverage } }, null, 2));
