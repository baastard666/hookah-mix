...
export default async function Result({ params }: { params: Promise<{ id: string }> }) {
  ...
  const componentNames = Object.fromEntries(components.map(component => [String(component.flavorId), `${component.brandName} ${component.flavorName}`]));
  ...
  return <main className="container section">
    ...
    <div className="result-head"><div><div className="eyebrow">Результат анализа</div><h1>{mix.title}</h1><p className="lead">{mix.components.map(component => `${component.flavor.brand.name} ${component.flavor.name} — ${component.percentage}%`).join(" · ")}</p></div><div className="big-score">{analysis.compatibility.compatibilityScore}<small style={{ fontSize: 18 }}>/10</small></div></div>
