...
import { presentationLabel, priorityLabels, reasonLabels, statusLabels, typeLabels } from "./recommendation-presentation";
...
export default function RecommendationSection({ actions, status, suggestedVariant, componentNames, proposalComparison, currentBuilderHref, proposedBuilderHref }: { actions: readonly PublicRecommendation[]; status: RecommendationStatus; suggestedVariant?: SuggestedMixVariant; componentNames: Readonly<Record<string, string>>; proposalComparison?: MixResultPresentation["proposalComparison"]; currentBuilderHref: string; proposedBuilderHref?: string }) {
  return <section className="card recommendation-section">
    ...
    <div className="recommendation-list">{actions.map(item => {
      ...
      return <article className="recommendation-item" key={`${item.type}-${item.componentIds.join("-")}-${item.directionLabels.join("-")}`}>
        ...
        {item.directionLabels.length > 0 && <p><b>Направление:</b> {item.directionLabels.join(", ")}</p>}
        {range && item.rangeLabel && <p><b>{item.rangeLabel}:</b> {range.min}–{range.max}%</p>}
        {item.proposedActualMixRole && <p><b>Роль в предложенном миксе:</b> {item.proposedActualMixRole}</p>}
        {action.type === "REBALANCE_COMPONENTS" && action.adjustments.map(adjustment => <p key={adjustment.componentId}><b>{componentNames[adjustment.componentId] ?? "Выбранный компонент"}:</b> {adjustment.suggestedPercentageRange.min}–{adjustment.suggestedPercentageRange.max}%</p>)}
    ...
    <div className="result-actions">{proposedBuilderHref && <Link className="btn btn-primary" href={proposedBuilderHref}>Применить предложенные пропорции</Link>}<Link className="btn btn-secondary" href={currentBuilderHref}>Изменить состав</Link></div>
  </section>;
}
