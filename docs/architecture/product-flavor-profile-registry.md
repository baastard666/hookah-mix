...
- Покрыты 45 из 86-87 `RESOLVED` продуктов (батчи 1–3 по 15); остальные — предмет будущих итераций (batch 4+).
- Целевая модель — 7 из 18 будущих Prisma-полей `Flavor` (ADR-014); остальные 11 явно исключены из registry, а не отложены. Все 45 уже покрытых продуктов доведены до полной 7-полевой модели (166 из 315 возможных значений заполнено); для двух производителей batch 1–2 (Hook, Husky) `heatResistance` не найден ни для одного продукта, `juiciness` заполнен только там, где источник явно использует слово «сочный».
