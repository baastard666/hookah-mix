...
export const normalizeTobaccoRow = (row: RawTobaccoRow, workbookName: string): NormalizedTobaccoStagingRecord => {
  ...
  const preliminaryText = normalizeImportText(row.cells.preliminaryValue); const preliminaryNumber = parseImportNumber(row.cells.preliminaryValue);
  if (preliminaryNumber !== null || preliminaryText?.toLocaleLowerCase("ru-RU").includes("предваритель")) derivedCharacteristics.push({ characteristic: "preliminaryValue", kind: "PRELIMINARY_INFERENCE", value: preliminaryNumber ?? preliminaryText!, sourceType: "DERIVED", source, ...(sourceReference ? { sourceReference } : {}), confidence: "LOW", importedFrom, originalValue: row.cells.preliminaryValue, notes: "Предварительное предположение, не сенсорный факт." });
