...
describe("canonical catalog identity mapper", () => {
  ...
  it("maps MANUFACTURER_ONLY to a line-less ASCII ID", () => { expect(mapIdentityResolutionToPersistedIdentity(resolveCatalogTobaccoIdentity({ brand: "Darkside", name: "Кокос" })).identity).toMatchObject({ manufacturerId: "darkside", productLineId: null, canonicalProductId: "darkside-manufacturer-only-kokos", identityStatus: "MANUFACTURER_ONLY" }); });
...
describe("canonical product IDs", () => {
  ...
  it("transliterates Unicode display names to ASCII", () => { expect(createResolvedCanonicalProductId("darkside-core", "Кокос 🥥")).toBe("darkside-core-kokos"); });
  it("transliterates Cyrillic", () => { expect(createManufacturerOnlyCanonicalProductId("blackburn", "Ваниль")).toBe("blackburn-manufacturer-only-vanil"); });
  it("normalizes case", () => { expect(createManufacturerOnlyCanonicalProductId("DARKSIDE", "КОКОС")).toBe("darkside-manufacturer-only-kokos"); });
...
describe("catalog identity backfill", () => {
  ...
  it("writes in apply mode", async () => { const store = memoryStore([record()]); await runCatalogIdentityBackfill(store, { apply: true }); expect(store.snapshot()[0]).toMatchObject({ identityStatus: "MANUFACTURER_ONLY", canonicalProductId: "darkside-manufacturer-only-kokos" }); });
...
describe("migration and consistency", () => {
  ...
  it("reports valid persisted identity", () => { const valid = record({ manufacturerId: "darkside", canonicalProductName: "Кокос", canonicalProductId: "darkside-manufacturer-only-kokos", identityStatus: "MANUFACTURER_ONLY" }); expect(verifyPersistedCatalogIdentities([valid])).toEqual({ valid: true, checked: 1, issues: [] }); });
  it("reports duplicate persisted IDs", () => { const values = { manufacturerId: "darkside", canonicalProductName: "Кокос", canonicalProductId: "darkside-manufacturer-only-kokos", identityStatus: "MANUFACTURER_ONLY" as const }; expect(verifyPersistedCatalogIdentities([record(values), record({ ...values, catalogId: 2 })]).issues.some(issue => issue.code === "DUPLICATE_CANONICAL_PRODUCT_ID")).toBe(true); });
