...
Первый operational batch P1 завершён в `src/lib/tobacco-identity-decisions/p1-decisions-batch-1.ts`: рассмотрены все 20 P1 group IDs, 19 продуктов получили canonical identity, а `MustHave / Ананас` сохранён как `MANUFACTURER_ONLY` без guessed alias к отдельному `Pineapple Rings`. Первый P0 batch завершён в `src/lib/tobacco-identity-decisions/p0-decisions-batch-1.ts`: 15/15 exact identities подтверждены evidence. Второй P0 batch хранится в `src/lib/tobacco-identity-decisions/p0-decisions-batch-2.ts`: из 15 групп 12 подтверждены, а `Duft / Papaya`, `Element / Feijoa Lemonade` и `Element / Персик` оставлены `AMBIGUOUS` из-за нескольких подтверждённых product lines. Третий P0 batch хранится в `src/lib/tobacco-identity-decisions/p0-decisions-batch-3.ts`: 15/15 exact identities подтверждены, без guessed product lines. Отдельные batch registry не создавались, authoritative aggregate объединяет P1 и три P0 batch. Следующий identity-шаг — следующая ограниченная ручная P0-партия; P2/P3 по-прежнему не применять. Следующая продуктовая итерация в roadmap — v0.3.3 Product Flavor Taxonomy Foundation.
...
Задача: продолжить operational часть v0.3.2 — следующая ограниченная ручная партия P0 identity records (batch 4).

Цель:
- открыть локальный reports/tobacco-identity-review-p0-p1.json или CSV;
- исключить уже рассмотренные P1 и P0 batch 1–3, затем выбрать следующую партию P0 детерминированно;
...
- подтвердить исходный review plan P0=76, P1=20, USER_PRIORITY=3 и отдельно учесть versioned decisions P1=20, P0 batch 1=15, P0 batch 2=15, P0 batch 3=15;
...
Последний подтверждённый baseline текущей ветки v0.3.2 после P0 batch 3:

- 740 tests passed, 20 test files;
...
Исторический baseline после unresolved report в v0.3.1: 500 tests. Baseline после ASCII-стандартизации: 689 tests; после P0 batch 2: 714 tests; после P0 batch 3: 740 tests.

## 16. Known issues

- Без decision layer исходная coverage остаётся низкой: components имеют 0 resolved, 21 manufacturer-only, 92 unresolved.
- После aggregate P1 + P0 batch 1–3: components 63 resolved, 2 manufacturer-only, 48 unresolved; VERIFIED components 62/1/34.
- В 208 exact groups отсутствует `productLine`; отсутствие линии не всегда ошибка.
- Исходный review plan по-прежнему генерируется как 99 `UNREVIEWED`; отдельно в versioned registry сохранены 20 P1 decisions и по 15 decisions в P0 batch 1–3 (batch 2: 12 resolved, 3 ambiguous; batch 3: 15 resolved).
