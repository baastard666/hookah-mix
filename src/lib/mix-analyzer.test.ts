import { describe,expect,it } from "vitest"; import { analyzeMix,validateMix,type AnalysisOptions } from "./mix-analyzer"; import type { FlavorData,MixInput } from "./types";
const flavor=(name:string,p:Partial<FlavorData>={},notes:string[]=[]):FlavorData=>({id:Math.random(),name,slug:name.toLowerCase(),description:"",brand:{name:"Test"},strength:5,heatResistance:7,intensity:5,sweetness:5,acidity:2,cooling:0,creaminess:0,bitterness:1,dryness:2,juiciness:5,freshness:3,dessertLevel:3,spiceLevel:0,floralLevel:0,herbalLevel:0,smokyLevel:0,naturalness:5,persistence:5,profileStatus:"DRAFT",profileSource:"MANUAL",notes:notes.map((n,i)=>({name:n,slug:n.replace(/\s+/g,"-"),category:"OTHER",intensity:10-i,noteType:i?"SECONDARY":"DOMINANT"})),...p});
