...
export const validateExpertMixImport = (tobacco: readonly NormalizedTobaccoStagingRecord[], mixes: readonly NormalizedMixStagingRecord[], components: readonly NormalizedMixComponentStagingRecord[], options: ExpertMixImportOptions): readonly ExpertMixImportIssue[] => {
  ...
  for (const item of tobacco) {
    ...
    for (const derived of item.derivedCharacteristics) if (derived.kind === "PRELIMINARY_INFERENCE" && derived.confidence !== "LOW") issues.push({ code: "PRELIMINARY_VALUE_WITHOUT_LOW_CONFIDENCE", severity: "ERROR", sheet: item.raw.sheet, rowNumber: item.raw.rowNumber, entityId: item.stagingId, field: derived.characteristic, originalValue: derived.originalValue, message: "Preliminary inference обязан иметь LOW confidence." });
  }
