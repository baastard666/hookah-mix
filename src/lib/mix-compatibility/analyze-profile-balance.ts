...
export function analyzeProfileBalance(result:MixProfileResult):AnalysisBlock{
  ...
  // ADR-015: bitterness, dryness, dessertLevel, creaminess, cooling, floralLevel, spiceLevel, smokyLevel are
  // secondary fields and may be null ("not measured"). Every rule below that reads one of them requires an
  // explicit `!== null` guard first - an unmeasured field must never silently satisfy or fail a threshold.
  ...
  if(p.dessertLevel!==null&&p.creaminess!==null&&p.dessertLevel>=T.high&&p.sweetness>=T.high&&p.creaminess>=T.highCreaminess)add("profile.dense-dessert","caution",-.5,"Плотный десертный профиль","Десертный профиль может оказаться слишком плотным",["dessertLevel","sweetness","creaminess"]);
  ...
  if(p.dryness!==null&&p.freshness>=T.moderateFreshness&&p.juiciness>=T.moderateJuiciness&&p.dryness<=T.maxComfortableDryness)add("profile.fresh-juicy","positive",.4,"Свежий сочный профиль","Профиль должен ощущаться сочным и свежим",["freshness","juiciness","dryness"]);
  ...
  if(p.creaminess!==null&&p.acidity>=8&&p.creaminess>=7)add("profile.acidity-creaminess","caution",-.5,"Кислотность и сливочность","Высокая кислотность может спорить со сливочной основой",["acidity","creaminess"]);
  if(p.bitterness!==null&&p.bitterness>=7&&p.acidity>=7)add("profile.bitterness-acidity","conflict",-.8,"Горечь и кислотность","Есть риск резкого сочетания высокой горечи и кислотности",["bitterness","acidity"]);
  ...
  if(p.smokyLevel!==null&&p.smokyLevel>=8&&p.freshness>=8)add("profile.smoky-fresh","caution",-.4,"Дымность и свежесть","Дымный профиль может спорить с выраженной свежестью",["smokyLevel","freshness"]);
